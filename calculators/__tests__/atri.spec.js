...
const goldenMasterFilePath = join(
  __dirname,
  './calculators_output/3fec2e9e8b9c54dd7a11f197d85ad4f6ce202654/atri.AlbanyCounty.ndjson.xz'
);

const goldenMaster = JSON.parse(
  execSync(
    `xzcat ${goldenMasterFilePath} | jq -s '. | map({ (.tmc): .atri }) | add'`
  )
);
    ...
    'Albany County 2017',
      ...
      each(
        ...
        (line, next) => {
          ...
          expect(atri).toEqual(goldenMaster[tmc]);
